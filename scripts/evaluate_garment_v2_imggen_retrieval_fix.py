...
class LazyImageDataset(Dataset):
    ...
    def __init__(self, imagefolder: str,
        ...
        _output_dir = os.listdir('./runs/close_image_scan_cg_retrieval/vis_new')
        all_images = []
        for item in _dir:
            if (item.endswith('.png') or item.endswith('.jpg') or item.endswith('.jfif')):
                item_name = f'valid_garment_{item.rsplit('.', 1)[0]}'
                if item_name not in _output_dir:
...
def main(args):
    ...
    parent_folder = os.path.join(args.log_base_dir, args.exp_name, f'{dataset_name}_cg_retrieval')
    ...
    for i in range(len_val_dataset):    
        ...
        question2 = 'Can you modify the description based on the geometry features of the garment worn by the model in this image in the Json format?'
        question3 = 'Can you modify the description to totally based on the new geometry feartures of the garment worn by this image in the Json format?'
        question4 = 'Can you estimate the sewing pattern code based on the image and Json format garment geometry description?'
        visualizations = []
        ...
        for k in range(len(questions)):
            ...
            if k < 2:
                image_clip = data_item['closest_images'][k]
            else:
        ...
        with open(os.path.join(saved_dir, 'output.txt'), 'w') as f:
            ...
            f.write('\nDESCRIPTION\n')
            f.write('Provide each of reference (1,2), then image, and then image and generated JSON description')
            f.write('\nTEXT OUTPUT\n')
            f.write(text_output)
            f.write('\nJSON OUTPUT\n')
