...
FLOAT_NUMBER = 44 # ao dai dataset
...
def generate_all_float_labels(all_floats, indices):
    if isinstance(all_floats[0], list) or len(indices) > 1:
        ...
        all_floats_padded = np.zeros(FLOAT_NUMBER)
        indices_i = indices[0]
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        for sample in list_data_dict:
            # Convert 'conversation' → 'conversations' for training format
            if "conversation" in sample and "conversations" not in sample:
                sample["conversations"] = sample.pop("conversation")

            # Convert 'all_floats' from string to list if needed
            if "all_floats" in sample and isinstance(sample["all_floats"], str):
                sample["all_floats"] = json.loads(sample["all_floats"])
        ...
        with open('/workspace/ao_dai_example/all_float_paths.json', 'r') as f:
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        # Initialize indices variable to avoid UnboundLocalError
        indices = []
        ...
        if 'all_floats' in sources[0]:
            has_floats = True
            # Only call generate_all_float_labels if indices is not empty
            if indices and len(indices) > 0:
                all_floats, all_floats_weight = generate_all_float_labels(sources[0]['all_floats'], indices)
            else:
                # Handle case when no sewing pattern indices are available
                # Create tensors with the right shape - the model expects (seq_len, FLOAT_NUMBER)
                # We'll let the data collator handle the proper reshaping
                all_floats = np.zeros(FLOAT_NUMBER)  # FLOAT_NUMBER-dimensional feature vector
                all_floats_weight = np.zeros(FLOAT_NUMBER)
...
def translate_args(model_args, data_args, training_args):
    args = edict(
        ...
        sample_rates=None,
        log_base_dir='./runs',
        exp_name="chatgarment_finetune_aodai",
        epochs=2,
        steps_per_epoch=2,
        batch_size=4,
...
def train(attn_implementation=None):
    ...
    data_path_list = {   
        "ao_dai": [
            '/workspace/ao_dai_example/ao_dai.json'
        ],
    }

    train_dataset, _, collate_fn = make_supervised_data_module(tokenizer=tokenizer,
    ...
    # resume = '/workspace/ChatGarment/checkpoints/try_7b_lr1e_4_v3_garmentcontrol_4h100_v4_final/pytorch_model.bin'
    # if args.resume:
    #     args.resume = resume
